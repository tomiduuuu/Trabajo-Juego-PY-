...
# Importar las secciones del juego
...
def main():
    ...
    pygame.display.set_caption("Voxel World 2D - Con Sprites")
    ...
    tooltip_manager = ui.TooltipManager(font_ui)
    ...
    while running:
                ...
                if game_state == "playing":
                    ...
                    if event.key == pygame.K_SPACE or event.key == pygame.K_w:
                        if not player.inventory_open: 
                            player.jump()
                            player.stop_mining()  # Detener minería al saltar
                    ...
                    if event.key == pygame.K_e:
                        ...
                        if player.inventory_open:
                            player.stop_mining()  # Detener minería al abrir inventario
                    
                    if event.key >= pygame.K_1 and event.key <= pygame.K_9:
                        player.hotbar_selected = event.key - pygame.K_1 
                        player.stop_mining()  # Detener minería al cambiar herramienta
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if game_state == "playing":
                    ...
                    if player.inventory_open:
                        ...
                        if event.button == 1: # Clic izquierdo - comenzar a minar
                            block_id = world_utils.get_block(world_data, gx, gy)
                            if block_id != s.BLOCK_AIR:
                                current_mining_block = (gx, gy)
                                mining = True
                                
                        if event.button == 3: # Clic derecho - colocar
                            ...
                            if block_to_place:
                                ...
                                player.stop_mining()  # Detener minería al colocar bloque
                
            # --- Soltar clic del ratón ---
        ...
        if game_state == "playing":
            ...
            # Actualizar minería si está minando
            if mining and current_mining_block:
                block_mined = player.update_mining(dt, world_data, current_mining_block)
                if block_mined:
                    # El bloque fue minado completamente
                    gx, gy = current_mining_block
            ...
            for enemy in enemies[:]:
                ...
                if enemy.rect.colliderect(player.rect) and not player.invincible:
                    ...
                    player.stop_mining()  # Detener minería al recibir daño
        ...
        # Obtener información de minería para dibujar grietas
        mining_target, mining_progress = player.get_mining_info()
        world_utils.draw_world(screen, world_data, offset_x, offset_y, mining_progress, mining_target)
        
        # Dibujar barra de progreso de minería
        if game_state == "playing" and not player.inventory_open:
            ui.draw_mining_progress(screen, player, offset_x, offset_y)
        ...
        if game_state == "playing":
            ui.draw_hud(screen, player, font_ui, tooltip_manager)
            if player.inventory_open:
                ui.draw_inventory_screen(screen, player, font_ui, font_title, crafting_ui, tooltip_manager)
                tooltip_manager.draw(screen, is_inventory_open=True)
            else:
                # Tooltip PEQUEÑO para bloques en el mundo
                mx, my = pygame.mouse.get_pos()
                gx = (mx + offset_x) // s.TILE_SIZE
                gy = (my + offset_y) // s.TILE_SIZE
                if 0 <= gx < s.WORLD_WIDTH and 0 <= gy < s.WORLD_HEIGHT:
                    block_id = world_utils.get_block(world_data, gx, gy)
                    if block_id != s.BLOCK_AIR:
                        block_name = s.BLOCK_NAMES.get(block_id, "Desconocido")
                        tooltip_manager.set_tooltip(block_name)
                
                tooltip_manager.draw(screen, is_inventory_open=False)
        elif game_state == "class_selection":
